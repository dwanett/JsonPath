...
В На конце каждого jsonPath можно указать функцию. Сейчас их 2.
...
В примере 11, по условию `price == 100` найдется 2 объекта, а условие `author == "Федор Достоевский"` внесет уточнение. В результате останется тот объект, который имеет `price == 100` и `author == "Федор Достоевский"`.
...
В примере 12 используется логическое ИЛИ. Отличие от примера 11 в том, что в примере 12 найдено 2 элемента `books`.
...
Таб. 14. Приоритет операций в примере 13.
...
Таблица 16. Приоритет операций в примере 14.
...
Таблица 17. Пример конвертации результата с операциями `allMatch` и `nonMatch`.
