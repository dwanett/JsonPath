...
public class Tests {
    ...
    public void PathTest() {
        ...
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.*.*"), "1225", "*.*.*.*");
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.details.*"), "1225", "*.*.details.*");
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.*.details"), "null", "*.*.*.details");
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.*[(user)].*"), "Alex", "*.*.*[(user)].*");
    ...
    public void FunctionTest() {
        ...
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.reviews.*.size()"), "1", "*.*.reviews.*.size()");
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.reviews.name()"), "[\"awards\",\"rating\",\"comment\",\"user\"]", "*.*.reviews.name()");
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.details.name()"), "[\"pages\",\"language\",\"available_copies\"]", "*.*.details.name()");
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.reviews[~].rating.distinct()"), "[4,5]", "*.*.reviews[~].rating.distinct()");
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.*[(rating)][~].rating.distinct()"), "[4,5]", "*.*.*[(rating)][~].rating.distinct()");
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.*[(rating)].awards.name()"), "[\"name\",\"type\",\"prize\"]", "*.*.*[(rating)].awards.name()");
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.*[(rating)].awards.distinct()"), "[{\"name\":\"Best Debut\",\"type\":\"special award\",\"prize\":500},{\"name\":\"Book of the Year\",\"type\":\"main award\",\"prize\":1500},{\"name\":\"Readers’ Choice\",\"type\":\"popular award\",\"prize\":300}]", "*.*.*[(rating)].awards.distinct()");
        softAssert.assertEquals(JsonPath.getValue(json, "*.*.*[(user == \"Anna\")][~].*.prize.distinct()"), "[1500]", "*.*.*[(user == \"Anna\")][~].*.prize.distinct()");
